...
function Application(screen, options) {
  ...
  this.listIssues = function(invalidate, focus) {
    ...
    list.on('select', function(num) {
      app.viewIssue(list.getSelected());
    });
    ...
    list.key('C-r', function() {
      app.listIssues(true, list.getSelected());
    });
  ...
  this.viewIssue = function(num, invalidate) {
    ...
    backend.lookup(num, config, app.cache, invalidate, function(err, issue) {
      ...
      app.empty();
      var single = views.single(screen, issue, app);
      ...
      single.on('issue.change', function(change) {
        app.loading();
        backend.change(
          num,
          change.changes,
