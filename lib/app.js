...
function Application(screen, options) {
  ...
  var setupKeyboard = function() {
    ...
    screen.key('S-c', function() { app.createIssue(); });
   ...
   */
  this.listIssues = function(force) {
    app.loading(force ? 'Refreshing...' : null);

    var list = views.list(screen, app, !!force, function(done) {
      return backend.query(config, app.storage, app.report, !!force, done);
    });
      ...
      async.doUntil(
        function(next) {
          screen.readEditor({ value: contents }, function(err, data) {
            ...
            backend.create(data, config, app.metadata, function(err, num) {
